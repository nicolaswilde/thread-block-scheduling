...
int main() {
    ...
    printf("================================================================================\n");
    printf("Test5 ==========================================================================\n");
    printf("K1 = K(%d, %d, %d, %d Bytes), all blocks 1s, stream1 (high priority)\n", b[0], t[0], r[0], s[0]);
    printf("K2 = K(%d, %d, %d, %d Bytes), all blocks 1s, stream2 ( low priority)\n", b[1], t[1], r[1], s[1]);
    printf("K3 = K(%d, %d, %d, %d Bytes), all blocks 1s, stream1 (high priority)\n", b[2], t[2], r[2], s[2]);
    printf("K4 = K(%d, %d, %d, %d Bytes), all blocks 1s, stream3 ( low priority)\n", b[3], t[3], r[3], s[3]);
    printf("================================================================================\n");
    ...
    checkCuda(cudaDeviceGetStreamPriorityRange(&priority_low, &priority_high));

    printf("Device Stream Priority: [%d(lowest), %d(highest)]\n", priority_low, priority_high);

    cudaStream_t stream[KN];
    checkCuda(cudaStreamCreateWithPriority(&stream[0], cudaStreamNonBlocking, priority_high));
    checkCuda(cudaStreamCreate(&stream[1]));
    checkCuda(cudaStreamCreate(&stream[2]));
    checkCuda(cudaStreamCreate(&stream[3]));
